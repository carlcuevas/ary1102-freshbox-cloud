...
- Figura 4. Instancias EC2 del Auto Scaling Group `freshbox-asg-app`
...
*Figura 1. Arquitectura TO-BE de FreshBox SpA, 3 capas sobre AWS (región us-east-1), con vista de red y de flujo de tráfico. Fuente: elaboración propia, `diagramas/diagrama-arquitectura-tobe.png`.*
...
La siguiente matriz aplica esos cuatro criterios a los 18 requerimientos identificados para el proyecto, funcionales y no funcionales, con el componente de AWS que implementa cada uno.

| ID | Requerimiento | Tipo | Impacto negocio | Riesgo si no se implementa | Depend. | Prioridad | Componente que lo implementa |
|---|---|---|---|---|---|---|---|
...
La distribución Multi-AZ está verificada en las tres capas: las seis subredes de `freshbox-vpc` se reparten en us-east-1a y us-east-1b, y el propio panel de la VPC confirma el CIDR 10.0.0.0/22 y el estado Available del recurso, como se observa en la Figura 2. El detalle individual de cada una de las seis subredes, con su zona de disponibilidad y su CIDR /26, está registrado en las capturas de `evidencias/01-vpc-subredes/`.
...
El Application Load Balancer `freshbox-alb` es internet-facing y está asociado a las dos subredes públicas (us-east-1a y us-east-1b); su Target Group `freshbox-tg-app` aplica un health check HTTP en la ruta `/` (Amazon Web Services, s.f.-g) y, como muestra la Figura 3, registra 2 targets en estado `Healthy`, uno en cada zona de disponibilidad. Es evidencia directa de que el tráfico puede seguir siendo atendido si una instancia —no una zona completa— deja de responder.

![Figura 3](../evidencias/recortes-informe/fig03-targets-healthy.png)

*Figura 3. Target Group `freshbox-tg-app`: los 2 targets registrados (`i-016e07318bb71bcbe` en us-east-1b e `i-0dd7f85d9dfc342ea` en us-east-1a) en estado Healthy. Fuente: `evidencias/06-alb-targetgroup/03-targets-healthy.png`.*
...
*Figura 4. Instancias EC2 del despliegue: las dos de la capa App distribuidas en us-east-1b y us-east-1a, y la de la capa Data, todas `t4g.small` con 3/3 status checks. El panel de detalle confirma la pertenencia al Auto Scaling Group `freshbox-asg-app`. Fuente: `evidencias/04-ec2-app/`.*

Según la bitácora técnica, en el mismo despliegue las instancias originalmente lanzadas por el grupo (`i-0cfb5552cdf4faecc` e `i-0d22849f21e783c74`) fueron reemplazadas por las instancias actuales sin intervención manual, restituyendo la capacidad deseada de dos instancias healthy, una por zona de disponibilidad. No se terminó ni detuvo una instancia a propósito para forzar esa prueba. El mecanismo responsable es el health check de tipo EC2 del Auto Scaling Group, que retira de servicio toda instancia que deja de estar en estado `running` (Amazon Web Services, s.f.-h); se verificó además que la plantilla `03-compute.yaml` no define `UpdatePolicy` ni `AutoScalingRollingUpdate`, descartando que el reemplazo se originara en una actualización progresiva de CloudFormation. El evento puntual que lo disparó no quedó capturado, precisamente por la brecha de observabilidad declarada en el punto 1.3.

Con el `HealthCheckType` del ASG configurado en `EC2` y no en `ELB`, ese mecanismo de reemplazo responde a fallas de la instancia detectadas por Amazon EC2, pero no se dispara si el Target Group marca un target unhealthy por un motivo propio de la aplicación: la misma brecha identificada en el punto 1.3, que se declara nuevamente al cierre de este punto para que ambas secciones sean coherentes entre sí.

La continuidad de los datos está cubierta por AWS Backup (Amazon Web Services, s.f.-i): el plan `freshbox-backup-plan-mysql` ejecuta la regla `respaldo-diario-mysql` a las 03:00 UTC, con una ventana de inicio de 60 minutos, una de finalización de 180 minutos y retención de 7 días sobre la instancia `freshbox-ec2-mysql`, como muestra la Figura 5.

![Figura 5](../evidencias/recortes-informe/fig05-backup-plan.png)

*Figura 5. Regla `respaldo-diario-mysql` del plan `freshbox-backup-plan-mysql`: frecuencia diaria por expresión cron, retención de 7 días, vault `freshbox-backup-vault`. Fuente: `evidencias/08-aws-backup/01-backup-plan-regla-diaria.png`.*

Una precisión importante sobre este punto: los recovery points de AWS Backup son regionales, no zonales, por lo que un respaldo tomado de la instancia en us-east-1a puede restaurarse en cualquier zona de la región, incluida us-east-1b. La subred `freshbox-sub-data-1b` está aprovisionada, pero hoy no aloja ninguna instancia de base de datos: la restauración en esa zona es una capacidad habilitada por el diseño de red, no algo que se haya ejecutado como prueba de recuperación en este proyecto. Esta distinción importa porque, al ser la base de datos una única instancia EC2 autoadministrada, la disponibilidad de la capa de datos depende hoy de una sola zona; ante la caída de us-east-1a, el ALB seguiría enrutando tráfico al target sano de 1b, pero ese target no podría completar operaciones porque no hay una instancia de base de datos activa en 1b. Este es precisamente uno de los motivos detrás de la recomendación de migrar a Amazon RDS Multi-AZ (punto 1.3), que sí provisiona automáticamente una réplica en espera en la segunda zona y falla sobre ella sin intervención manual (Amazon Web Services, s.f.-e).

### Validación de escalabilidad

El escalado horizontal está definido en `freshbox-asg-app` con mínimo 2, máximo 4 y capacidad deseada 2 instancias, sobre las subredes `freshbox-sub-app-1a` y `freshbox-sub-app-1b`. El aprovisionamiento de una instancia nueva es automático: el UserData del Launch Template instala Docker, autentica contra Amazon ECR (Amazon Web Services, s.f.-j) y despliega los 5 contenedores apuntando a la IP privada de la instancia de base de datos, sin intervención manual. Las 5 imágenes están efectivamente en ECR, como confirma la Figura 6, y las instancias App las descargaron desde ahí durante su aprovisionamiento, lo que verifica que el mecanismo fue ejercitado en la práctica y no solo configurado.

![Figura 6](../evidencias/recortes-informe/fig06-ecr-repos.png)

*Figura 6. Los 5 repositorios privados en Amazon ECR: `freshbox-frontend` y los cuatro microservicios del CRUD. Fuente: `evidencias/05-ecr/01-repositorios-listado.png`.*

Con esto dicho, hay dos límites que matizan la afirmación de "escalabilidad automática" tal como está desplegada hoy. Primero, el rango mín. 2 / máx. 4 es una **capacidad habilitada, no una capacidad activada**: al no existir una política de Target Tracking Scaling asociada, ningún aumento de CPU o de solicitudes por target dispara automáticamente el paso de 2 a 4 instancias; alguien tendría que cambiar la capacidad deseada manualmente. Segundo, los 5 contenedores —incluida la lectura del catálogo y las operaciones de administración— viven juntos en cada instancia App, y el Auto Scaling Group escala instancias completas, no microservicios individuales; no existe hoy un mecanismo que permita escalar la lectura del catálogo de forma independiente de las operaciones de creación, edición o borrado, aunque la arquitectura de microservicios sí deja esa evolución abierta para etapas futuras.
...
La segmentación en tres capas está reflejada en dos route tables distintas: `freshbox-rt-public`, asociada a las 2 subredes Web con salida a Internet vía Internet Gateway; y `freshbox-rt-private`, asociada a las 4 subredes App y Data con salida a Internet vía `freshbox-natgw` (Amazon Web Services, s.f.-d). La Figura 7 confirma en la consola que `freshbox-rt-private` tiene efectivamente 4 subredes asociadas explícitamente y que el NAT Gateway `freshbox-natgw` está en estado Available; las rutas `0.0.0.0/0` de cada tabla están definidas en la plantilla `01-red.yaml`. Solo el ALB es internet-facing: ninguna instancia de las capas App o Data tiene IP pública, como se aprecia en las columnas Public IPv4 de la Figura 4.

![Figura 7a](../evidencias/recortes-informe/fig07a-route-table-privada.png)

*Figura 7a. Route table `freshbox-rt-private` (`rtb-07d378b8b99c8e63d`) con 4 subredes asociadas explícitamente, frente a `freshbox-rt-public` con 2. Fuente: `evidencias/01-vpc-subredes/10-route-table-privada.png`.*

![Figura 7b](../evidencias/recortes-informe/fig07b-nat-gateway.png)

*Figura 7b. NAT Gateway `freshbox-natgw` en estado Available, con IP elástica pública 100.27.78.176, único punto de salida a Internet de las subredes privadas. Fuente: `evidencias/01-vpc-subredes/11-nat-gateway.png`.*
...
La consola corrobora ese diseño: el panel de detalle de `freshbox-sg-app` reporta **2 reglas de entrada** y el de `freshbox-sg-bd` reporta **1 regla de entrada**, coincidiendo exactamente con lo declarado en la plantilla, como muestra la Figura 8. La descripción de cada grupo documenta además el origen esperado.

![Figura 8a](../evidencias/recortes-informe/fig08a-sg-app.png)

*Figura 8a. `freshbox-sg-app` (`sg-00520ee631b9d68c7`): 2 reglas de entrada, descripción "solo trafico desde el ALB". Fuente: `evidencias/02-security-groups/02-sg-app-detalle.png`.*

![Figura 8b](../evidencias/recortes-informe/fig08b-sg-bd.png)

*Figura 8b. `freshbox-sg-bd` (`sg-07e9f0a7448e1e386`): 1 regla de entrada, descripción "solo MySQL desde SG-App". Fuente: `evidencias/02-security-groups/03-sg-bd-detalle.png`.*

Los volúmenes EBS de las capas App y Data están cifrados (`Encrypted: true` en las plantillas de cómputo); la administración es exclusivamente por SSM Session Manager, sin llaves SSH, y no existe ninguna regla de entrada al puerto 22 en los Security Groups; el rol de las instancias tiene permiso de solo lectura sobre ECR, mientras que el push de imágenes se realiza desde una identidad distinta; y la infraestructura completa es auditable y reproducible como código en los 5 stacks de CloudFormation (Amazon Web Services, s.f.-k).

Finalmente, la validación funcional end-to-end del CRUD de productos —crear, listar, editar y eliminar— se realizó a través del DNS público del ALB, tanto por API con `curl` como por la interfaz web real, y quedó registrada en la secuencia de capturas de la Figura 9: listado inicial con los 5 productos originales, creación de un producto nuevo (id 7, "NARANAX") y listado final tras eliminarlo, mostrando que se recupera exactamente el estado original. Esto confirma simultáneamente que la segmentación de red, los Security Groups encadenados y la corrección del primer defecto de la aplicación (rutas relativas de API) funcionan en conjunto, y no solo en la teoría del diseño.
...
*Figura 9. Validación CRUD end-to-end vía el frontend web servido por el ALB: (a) listado inicial con 5 productos, (b) producto creado vía POST ("NARANAX", id 7) y (c) listado final tras el DELETE, de vuelta al estado original. Fuente: `evidencias/07-validacion-crud/`.*

### Tabla de trazabilidad: requerimiento, componente y evidencia

Para cerrar el círculo entre lo declarado en el punto 1.4 y lo efectivamente desplegado, la siguiente tabla vincula cada requerimiento de prioridad Alta o Media ya implementado con el componente de AWS que lo satisface, el método de validación aplicado y la evidencia concreta que lo respalda.

| Requerimiento | Componente AWS que lo implementa | Cómo se validó | Evidencia |
|---|---|---|---|
| R01 / R02 CRUD del catálogo | 4 microservicios Node.js tras el ALB | GET/POST/PUT/DELETE end-to-end por DNS público, vía API y vía interfaz web | Figura 9 · `evidencias/07-validacion-crud/` |
...
| R06 BD aislada de Internet | Subred privada Data + `SG-bd` (3306 solo desde `SG-app`) | Regla única de entrada en `SG-bd`; ausencia de IP pública en la instancia de datos | Figuras 4 y 8b |
...
### Brechas reconocidas y su recomendación

Como corresponde a un análisis de arquitectura honesto, se declaran aquí las brechas conocidas del diseño actual, ya introducidas en el punto 1.3, junto con su recomendación:

- **HTTPS:** los Security Groups permiten el puerto 443, pero el ALB solo tiene listener HTTP:80, porque no hay un certificado ACM disponible en el entorno académico. *Recomendación:* agregar un listener 443 con certificado ACM, política TLS ≥ 1.2 y redirección 80→443.
- **NAT Gateway único** (`freshbox-natgw`, en us-east-1a): es un punto único de falla para la salida a Internet de ambas subredes privadas. *Recomendación:* un NAT Gateway por zona de disponibilidad, con su propia route table.
- **Base de datos autoadministrada** en una sola instancia EC2, sin réplica en la segunda zona: los parches, los respaldos y un eventual failover dependen del equipo. *Recomendación:* migrar a Amazon RDS for MySQL Multi-AZ.
- **Sin política de escalado dinámico:** el rango mín. 2 / máx. 4 del ASG existe, pero ninguna regla lo activa automáticamente. *Recomendación:* Target Tracking Scaling sobre CPU o `RequestCountPerTarget`.
- **Health check del ASG de tipo EC2 y no ELB:** un target unhealthy en el Target Group no dispara por sí solo el reemplazo de la instancia. *Recomendación:* cambiar el tipo de health check a `ELB` y apuntarlo a `/api/products`.
- **Sin observabilidad:** no hay CloudWatch Agent, logs centralizados de los contenedores ni alarmas. Esta brecha tuvo una consecuencia concreta documentada en el punto 1.3: el reemplazo automático de instancias funcionó, pero el equipo no pudo determinar automáticamente qué lo disparó. *Recomendación:* instalar el agente, crear un log group por microservicio y configurar alarmas sobre errores 5XX y CPU.
- **Dependencia de la IP privada de la base de datos:** el UserData recibe la IP en el momento de crear el stack; si la instancia de datos se recrea, las instancias App quedarían apuntando a una dirección inexistente. *Recomendación:* resolver la base de datos por nombre DNS privado o migrar al endpoint gestionado de RDS.

Reconocer estas brechas —en lugar de presentar la arquitectura como si no tuviera limitaciones— es, precisamente, el criterio que se espera de un arquitecto cloud: cada una está justificada por una restricción real del entorno académico o por el alcance explícito de esta etapa del proyecto, y cada una tiene una ruta de mejora concreta para cuando el proyecto avance más allá del EP1.
